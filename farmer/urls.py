...
urlpatterns = [
    ...
    path("land/",views.FarmInfoCreateView.as_view(),name=views.FarmInfoCreateView.name,), # 농지정보 수정, 삭제하기
    path('land/total-area/', TotalLandAreaAPIView.as_view(), name='total-land-area'),#TODO: 농지 총 면적 lands에 추가
    path("land/<uuid:uuid>/",views.FarmInfoAPIView.as_view(),name=views.FarmInfoListView.name,),
]
