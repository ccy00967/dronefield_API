...
from common.utils.naver_map import naver_map
...
@require_GET
def user_naver_map_view(request, uuid):

    farminfo = FarmInfo.objects.get(uuid=uuid)
    
    farminfo_jibun = farminfo.jibun

    return naver_map(address = farminfo_jibun, request = request)

    # VWorld API 키 및 기본 URL 설정

@require_GET
def address_naver_map_view(request, address): 
    return naver_map(address = address, request = request)
